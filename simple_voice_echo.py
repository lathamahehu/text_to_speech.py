...
# Doraemon Image Path
DORAEMON_IMAGE_PATH = 'doraemon_face.png' # Make sure this file is in the same directory!
DORAEMON_IMAGE_SIZE = (40, 40) # Adjust size as needed
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    self.message_queue.put(f"RECOGNIZED: {text.lower()}") # Send recognized text to main thread

                except sr.WaitTimeoutError:
                    pass # Continue waiting quietly
...
class PygameVoiceEcho:
    def __init__(self):
        ...
        try:
            ...
            self._add_status_message(f"INFO: Loaded {DORAEMON_IMAGE_PATH}")
        except pygame.error as e:
            self._add_status_message(f"ERROR: Could not load Doraemon image: {e}. Ensure '{DORAEMON_IMAGE_PATH}' exists.")
            self.doraemon_image = None
    ...
    def _initialize_browser_new_instance(self, browser_type='chrome'):
        """Initializes a BRAND NEW Selenium WebDriver instance, closing any existing one first."""
        
        # Explicitly close any existing browser before opening a new one
        if self.browser_driver:
            self._add_status_message("ACTION: Closing previous browser instance...")
            try:
        ...
        self._add_status_message(f"ACTION: Opening NEW {browser_type.capitalize()} browser for display...")
        try:
            ...
            self._add_status_message(f"STATUS: NEW {self.browser_driver.name.capitalize()} browser launched successfully.")
            return True
    ...
    def display_speech_in_browser(self, text_to_display):
        ...
        if not self._initialize_browser_new_instance('chrome'): # Opens a new Chrome instance
            self._add_status_message("ERROR: Could not open a new browser to display speech.")
            return False
        ...
        """
        try:
            self._add_status_message(f"ACTION: Displaying speech in NEW browser: '{text_to_display[:30]}...'")
            self.browser_driver.get(f"data:text/html;charset=utf-8,{html_content}")
            return True
        except WebDriverException as e:
            self._add_status_message(f"ERROR: Failed to display speech in new browser: {e}")
            self._add_status_message("INFO: Browser might have closed immediately. Trying to re-open next time.")
    ...
    def close_browser_driver(self):
        ...
        if self.browser_driver:
            ...
            try:
                ...
                # Fixed typo: self._add_status_message
